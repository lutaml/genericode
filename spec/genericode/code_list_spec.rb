    ...
    Dir.glob(File.join(xml_fixtures_dir, '*.gc')).each do |xml_file|
      ...
      context "with #{base_name}.{gc,gcj}" do
        ...
        it 'converts JSON to XML correctly' do
          ...
          # Parse the generated XML and verify semantic content matches the JSON
          reparsed = described_class.from_xml(generated_xml)
          expect(reparsed.identification.short_name.content).to eq(code_list.identification.short_name.content)
          expect(reparsed.column_set.column.size).to eq(code_list.column_set.column.size)
          expect(reparsed.simple_code_list.row.size).to eq(code_list.simple_code_list.row.size)
        end
